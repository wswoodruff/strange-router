...
it.only('renders without crashing with empty routes', () => {

    renderWithRouter('/', <Routes routes={[]} />);
});

it.only('renders a simple route', () => {
...
it.only('renders different routes given an array of routes', () => {
...
it.only('renders childRoutes many levels deep', () => {
...
it.only('redirects "to" a fallback route if no other routes match', () => {
...
// it.only('Internally consolidates routes with path "/"', async () => {
...
it.only('waits for "pre" to resolve before rendering a route', async () => {
...
it.only('renders "fallback" when specifying a "pre" function', async () => {
...
it.only('renders "fallback" as children in "component" when "wrapFallbackWithComponent" is true, waits to render path children until "pre" resolves', async () => {
...
it.only('redirects "from" a specific route "to" another', () => {
