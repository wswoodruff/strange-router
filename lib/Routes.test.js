...
const strangeAfterEach = (testName) => {
...
const runIt = async (name, func) => {

    try {
        await it(name, func);
    }
    catch (err) {
        console.log(`Test "${name}" failed with: ${err}`);
    }

    strangeAfterEach(name);
};

// Ooh. Nice.
runIt.only = it.only;

runIt('renders without crashing with empty routes', () => {

    renderWithRouter('/', <Routes routes={[]} />);
    strangeAfterEach('renders without crashing with empty routes');
});

runIt('renders a simple route', () => {
...
runIt('renders different routes given an array of routes', () => {
...
runIt('passes childRoute as "children" prop', () => {
...
runIt('renders childRoutes many levels deep', () => {
...
runIt('redirects "to" a fallback route if no other routes match', () => {
    ...
    expectHistoryPathnameEquals(history, '/login');
});

// it.only('Redirects to nearest ancestor redirect or "/404" for non-matching paths', () => {
...
runIt('Internally consolidates routes with path "/"', () => {
...
runIt('waits for "pre" to resolve before rendering a route', async () => {
...
runIt('renders "fallback" when specifying a "pre" function', async () => {
...
runIt('renders "fallback" as children in "component" when "wrapFallbackWithComponent" is true, waits to render path children until "pre" resolves', async () => {
...
runIt('redirects "from" a specific route "to" another', () => {
