...
internals.decorateRoutes = ({ routes, basePath = '/', defaultRedirect = '/404' }) => {
    ...
    //////////////
    // Setup redirects going all the way up to the top
    if (routes.length > 0 && !isFallbackRedirect(lastChild(routes))) {
        let fallbackRedirectPath = defaultRedirect;

        if (routes[0].parentRoute) {
            // console.log('routes[0].parentRoute.path', routes[0].parentRoute.path);
            // console.log('routes[0].parentRoute.parentRoute?.childRoutes', routes[0].parentRoute.parentRoute?.childRoutes);
            // console.log('routes', routes);
            // console.log('routes[0].parentRoute.childRoutes', routes[0].parentRoute.childRoutes);
            // The parentRoute's last sibling will be a fallback redirect
            fallbackRedirectPath = lastChild(routes[0].parentRoute.childRoutes)?.redirect?.to;

            console.log('fallbackRedirectPath', fallbackRedirectPath);
        }

        routes.push({ redirect: { to: fallbackRedirectPath || defaultRedirect } });
    }

    // console.log('routes after adding fallback redirects', routes);
    // console.log('And the childRoutes', routes.map((route) => route.childRoutes));
        ...
        if (route.childRoutes) {
            route.childRoutes = (
                decorateRoutes({
                    ...
                    routes: route.childRoutes.map((childRoute) => ({
                        ...childRoute,
                        parentRoute: route
                    }))
                })
